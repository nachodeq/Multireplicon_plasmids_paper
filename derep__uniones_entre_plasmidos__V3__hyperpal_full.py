...
for hw in HALF_WINDOWS:
    ...
    # --- REAL BREAKPOINTS ---
    for i,(chrom,pos,L) in enumerate(breakpoints):
        fasta=get_fasta_path(chrom)
    ...
    bp_rate=bp_hits/len(jobs_bp)
    bg_rate=bg_hits/len(jobs_bg)
    chr_rate=chr_hits/len(jobs_chr)

    enrichment = bp_rate/bg_rate if bg_rate>0 else 0

    print(f"Breakpoints:    {bp_hits}/{len(jobs_bp)}  ({bp_rate:.4f})")
    print(f"Random plasmid: {bg_hits}/{len(jobs_bg)}  ({bg_rate:.4f})")
    print(f"Chromosome:     {chr_hits}/{len(jobs_chr)} ({chr_rate:.4f})")
    print(f"Enrichment BP/BG: {enrichment:.2f}x")
...
df.to_csv(os.path.join(BASE_DIR,"FULL_incremental_summary.tsv"),
          sep="\t",index=False)
