...
LIST = Path(BASE) / "coords99_with_hits.list"
NCORES = 30
OUT = Path(BASE) / "coords99_all_hits.tsv"

def parse_one(path_str):
    rows = []
    coords = Path(path_str)

    # replicon_pair = carpeta dos niveles arriba
    pair = coords.parents[2].name

    plasmA, plasmB = coords.stem.replace(".99","").split("__")
...
def main():
    ...
    print(f"Parsing {len(coords_files)} coords (delta99) with hits")
